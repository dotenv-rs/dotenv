//! This crate provides a configuration loader in the style of the [ruby dotenvx
...
/// After loading the dotenvx file, fetches the environment variable key from the current process.
///
...
///
/// let key = "FOO";
/// let value= dotenvx::var(key).unwrap();
/// ```
...
/// After loading the dotenvx file, returns an iterator of (variable, value) pairs of strings,
...
/// use std::io;
///
/// let result: Vec<(String, String)> = dotenvx::vars().collect();
/// ```
pub fn vars() -> Vars {
    START.call_once(|| {
        dotenv().ok();
...
/// dotenvx::from_path(my_path.as_path());
/// ```
...
///
/// let my_path = env::home_dir().and_then(|a| Some(a.join("/.env"))).unwrap();
/// let iter = dotenvx::from_path_iter(my_path.as_path()).unwrap();
///
/// for item in iter {
...
/// ```
/// use dotenvx;
/// dotenvx::from_filename("custom.env").ok();
/// ```
///
/// It is also possible to do the following, but it is equivalent to using `dotenvx::dotenvx()`,
/// which is preferred.
///
/// ```
/// use dotenvx;
/// dotenvx::from_filename(".env").ok();
/// ```
...
/// ```no_run
/// use dotenvx;
/// let iter = dotenvx::from_filename_iter(".env").unwrap();
///
/// for item in iter {
///   let (key, val) = item.unwrap();
///   println!("{}={}", key, val);
...
/// Like `dotenvx`, but returns an iterator over variables instead of loading into environment.
///
/// # Examples
/// ```no_run
/// use dotenvx;
///
/// for item in dotenvx::dotenv_iter().unwrap() {
///   let (key, val) = item.unwrap();
///   println!("{}={}", key, val);
/// }
/// ```
